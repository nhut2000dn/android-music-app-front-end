...
public class ChartsSongActivity extends AppCompatActivity implements ChartsSongAdapter.OnListenerRow, ChartsSongAdapter.OnListenerMenuOption {

  @BindView(R.id.title_tool_bar)
  TextView titleToolBar;

  @BindView(R.id.rv_charts_song)
  RecyclerView rvChartsSong;
  ...
  Call<AudioResponse> callChartsSong;

  public static List<Audio> listChartsSong = new ArrayList<>();
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(R.layout.activity_charts_song);
...
//                .fit().placeholder(R.drawable.playholder_music).into(imageSongBar);
  ...
  public void loadDataNetWord(int id) {
    callChartsSong = MainActivity.service.chartsSongs(id);
    callChartsSong.enqueue(new Callback<AudioResponse>() {
      @Override
      public void onResponse(Call<AudioResponse> call, Response<AudioResponse> response) {
        if (response.isSuccessful()) {
          assert response.body() != null;
          listChartsSong = response.body().getData();
          setUpAdapter();
        }
      }
  ...
  public void onClickRow(int position) {
    ...
    playAudio(position);
    togglePlayAudio.setImageResource(R.drawable.ic_pause_circle_outline);
    nameSongBar.setText(listChartsSong.get(position).getTitle());
    nameArtistBar.setText(listChartsSong.get(position).getArtist());
//    Picasso.get().load(listChartsSong.get(position).getImage())
//            .fit().placeholder(R.drawable.playholder_music).into(imageSongBar);
    Picasso.get().load("http://192.168.1.117/music_offical/storage/app/public/" + listChartsSong.get(position).getImage())
            .fit().placeholder(R.drawable.playholder_music).into(imageSongBar);
    startActivity(new Intent(this, IsPlayingActivity.class));
    overridePendingTransition( R.anim.slide_in_up, R.anim.no_change );
    ...
    popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
      public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.play_next) {
          ...
          intent.putExtra("id", listChartsSong.get(position).getId());
          intent.putExtra("name", listChartsSong.get(position).getTitle());
  ...
  public void playAudio(int audioIndex) {
    if (!checkPlayNewInStorage) {
      ...
      storage.storeAudio((ArrayList<Audio>) listChartsSong);
  ...
  private void setUpAdapter() {
    ChartsSongAdapter themeSongAdapter = new ChartsSongAdapter(listChartsSong, this, this);
    LinearLayoutManager layoutManager = new LinearLayoutManager(this);
    layoutManager.setOrientation(LinearLayoutManager.VERTICAL);

    rvChartsSong.setLayoutManager(layoutManager);
    rvChartsSong.setAdapter(themeSongAdapter);
  }
